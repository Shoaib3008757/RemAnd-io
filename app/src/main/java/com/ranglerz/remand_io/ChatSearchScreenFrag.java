...
public class ChatSearchScreenFrag extends ListFragment
{
    ...
    public static LocalService m_Service =null;  				 //our service
    StartupCommunicationActivity mActivity;  							 //reference to the hosting activity
    ...
    public void onAttach(Activity activity)
    {
        ...
        mActivity = (StartupCommunicationActivity)getActivity(); //set a reference to the activity
    ...
    public void onStart()
    {
        ...
        if (m_Service !=null) //done for cases of fragment refresh done by the system
            UpdateListView();
    ...
    public boolean onContextItemSelected(MenuItem item)
    {
        ...
        switch(item.getItemId()) //switch by the selected operation:
        {
            case R.id.action_ignore_user:
            {
                ...
                m_Service.mBannedFromPrivateChatUsers.put(unique, "true"); //update the ignore list
                return true;
            }
            case R.id.action_unignore_user:
            {
                ...
                m_Service.mBannedFromPrivateChatUsers.remove(unqiue); //remove the user from the ignore list
                return true;
            }
            ...
            {
                mActivity.historyFrag.OnContextMenuItemSelected(item);
                return true;
            }
        }
        return true;
...
//					}
        ...
        new AlertDialog.Builder(mActivity)
                ...
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        mActivity.killService();  //close the entire app
        ...
        switch (eventCode) //figure out which wifi p2p event has occurred
        {
            ...
            case Constants.SERVICE_BROADCAST_WIFI_EVENT_P2P_DISABLED:
            {
                ...
                if (mIsWifiDirectEnabled==true)
                {
                    ...
                    Intent intent = new Intent(mActivity,StartupCommunicationActivity.class);
    ...
    public void ClearIgnoredUsersList()
    {
        if (m_Service !=null)
        {
            m_Service.mBannedFromPrivateChatUsers.clear(); //clear the banned users list
            UpdateListView();
        }
    ...
    public void onRefreshButtonClicked (View v)
    {
        ...
        if(mIsServiceBound && m_Service !=null)  //wifi direct is enabled
        {
            m_Service.OnRefreshButtonclicked();
            UpdateListView();
        }
    ...
    {
        if (m_Service ==null) //if we don't have a reference to the service yet
            return;
        ...
        synchronized (mListContent)
        {
            ...
            if (!m_Service.mActiveChatRooms.isEmpty()) //we want to look up all hosted chat rooms on this device
            {
                Collection<ActiveChatRoom> chatRooms = m_Service.mActiveChatRooms.values();  //get all available hosted chat rooms
            ...
            if (!m_Service.mDiscoveredChatRoomsHash.isEmpty()) //if there's a valid discovered chat room list available
            {
                Collection<ChatRoomDetails> chatRooms = m_Service.mDiscoveredChatRoomsHash.values();  //get all discovered chat rooms
                for (ChatRoomDetails room : chatRooms) //for each chat room
                {
                    ...
                    if (room.isPrivateChatRoom) //if this is a private chat
                    {
                        ...
                        if (room.hasNewMsg)
                            singleChatEntryView.put(Constants.HASH_MAP_KEY_SEARCH_NEW_MSG_ICON, Integer.toString(R.drawable.msg_icon));
                        if (m_Service.mBannedFromPrivateChatUsers.containsKey(room.RoomID))
                    ...
                    else //this is a public chat room, NOT hosted by us
                    {
                        ...
                        if (room.Password!=null) //if this room requires a pw
                        {
                            ...
                            if (m_Service.mActiveChatRooms.containsKey(room.RoomID))
