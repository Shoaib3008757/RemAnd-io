...
public class LocalService extends Service
{
            ...
            mRefreshHandler = new Handler()
            {
                ...
                public void handleMessage(Message msg)
                {
                    ...
                    else //it's a TO telling us to send discovery messages to all peers
                    {
                        ...
                        sendEmptyMessageDelayed(0, StartupCommunicationActivity.RefreshPeriodInMs);
                    }
                }
            };
    ...
    public void OnNewChatMessageArrvial (String[] msg, String peerIP)
    {
        ...
        if (msg[3].equalsIgnoreCase(StartupCommunicationActivity.ID)) //if the target room is our unique, then it's a private chat message
        {
        ...
        if (StartupCommunicationActivity.is_ToNotifyOnNewMsg &&
                (DisplayedAtChatActivity==null || !msg[3].equalsIgnoreCase(DisplayedAtChatActivity.RoomID)))
        {

            // Creates an explicit intent for an Activity in your app
            Intent resultIntent = new Intent(this, StartupCommunicationActivity.class);
            ...
            stackBuilder.addParentStack(StartupCommunicationActivity.class);
    ...
    public String ConvertMsgTextToSocketStringFormat(String msg, String chatRoomUnique)
    {
        ...
        toSend.append(StartupCommunicationActivity.user_Name +Constants.STANDART_FIELD_SEPERATOR);   //add self name
        toSend.append(StartupCommunicationActivity.ID +Constants.STANDART_FIELD_SEPERATOR);	//add unique
    ...
    private String ConstructJoinMessage (int opcode, String RoomUniqueID, String pw)
    {
        ...
        if (opcode==Constants.CONNECTION_CODE_PRIVATE_CHAT_REQUEST)  //this is a connection establishment for a private chat
        {
            ...
            ans.append(StartupCommunicationActivity.user_Name + Constants.STANDART_FIELD_SEPERATOR); //add the username
            ans.append(StartupCommunicationActivity.ID + "\r\n"); //add our uniqueID
        }
        else //this is a connection establishment for a public chat
        {
            ...
            ans.append(StartupCommunicationActivity.user_Name + Constants.STANDART_FIELD_SEPERATOR); //add the username
            ans.append(StartupCommunicationActivity.ID + Constants.STANDART_FIELD_SEPERATOR); //add the user's uniqueID
...
//    	}
    ...
    public void CreateNewHostedPublicChatRoom (String name, String password)
    {
        ...
        ChatRoomDetails newDetails = new ChatRoomDetails(
                StartupCommunicationActivity.ID + "_" + (++StartupCommunicationActivity.C_RoomAccumulatingSerialNumber),
    ...
    private void DeleteTimedOutRooms()
    {
        ...
        synchronized (mDiscoveredChatRoomsHash)
        {
            ...
            for (i=0; i<numOfRooms ;i++)
            {
                ...
                if (timeDiff >= StartupCommunicationActivity.RefreshPeriodInMs*Constants.TO_FACTOR) //if this room has time out
                {
    ...
    private String BuildUsersPublicationString()
    {
        ...
        StringBuilder res = new StringBuilder(Integer.toString(Constants.CONNECTION_CODE_PEER_DETAILS_BCAST) + Constants.STANDART_FIELD_SEPERATOR
                + StartupCommunicationActivity.user_Name + Constants.STANDART_FIELD_SEPERATOR
                + StartupCommunicationActivity.ID + Constants.STANDART_FIELD_SEPERATOR);
